...
#в начале тут ещё были сеттеры и гетттеры как в плюсах, но было принято решение писать как на питоне (поменять семантику)
#т.к. 1)надо один стиль
#     2)нельзя писать в питоне как на плюсах
# И мне всё кажется, что в питоне классы - это какие-то уродцы. Инкапсуляция за просто так нарушаетя. Полиморфизм дурной.
# Наследование норм, но без первых двух не имеет смысла. Фейсы писать нельзя:( Отдельно вынеси хоть что-то сложнее констант - нельзя.
# Питон как-то плохо присполоблен для ООП:(