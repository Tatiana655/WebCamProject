...
# мб есть сполоб написать это правильно, но мн е он не извествен
# (В любом случае мы код пишем не для того, чтобы он работал)
...
class Application:
    ...
    def __init__(self, output_path="./"):
        ...
        Application.button.append(tk.Button(self.root, text="NEXT",
                                            command=self.point))  # тут кнопки, бегунки? мб через глобальные переменные
        ...
        if ok:  # frame captured without any errors
            ...
            if Application.Mode == MODE[1]:  # reading
                ...
                if Application.count_click == 12:
                    ...
                    Application.data = [Application.Scroll[6].get(), Application.Scroll[7].get(),
                                        Application.Scroll[8].get()]
                    filter = cv2.inRange(frame, np.array(Application.min_color), np.array(Application.max_color))
                    st1 = cv2.getStructuringElement(cv2.MORPH_RECT, (Application.data[1], Application.data[1]),
                                                    (-1, -1))
                    st2 = cv2.getStructuringElement(cv2.MORPH_RECT, (Application.data[2], Application.data[2]),
                                                    (-1, -1))
    ...
    def point(self):
        ...
        Application.button[4].pack(side='left', padx=10, pady=10)
